...
with open('upload_test_change.csv', 'w', newline='') as csvfile:
    ...
    header = ["Sensor_number", "X", "Y", "Z", "Year", "Month", "Day", "Hour", "Minute", "Second", "Temperature", "Timestamp"]
    ...
    for year in range(2020, 2024):
        for sensor_number in range(1, 55):  # Sensor_number range [1, 54]
            ...
            while current_time.year == year:  # Generate data for the specified year
                ...
                data_row = [sensor_number, x, y, z, current_time.year, current_time.month, current_time.day, current_time.hour, current_time.minute, current_time.second, round(random.uniform(32, 38), 2), current_time.strftime('%Y-%m-%d %H:%M:%S')]
